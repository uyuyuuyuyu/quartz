...
**Argument:**
1.  **Assumption:** Start with a connected graph $G$ (representing villages and roads).
2.  **Cycle Detection:** If the graph contains a cycle (a loop where you can return to the start without backtracking), then there are multiple paths between vertices.
3.  **Redundancy Removal:** In a cycle, removing one edge does not disconnect the graph. The vertices remain reachable via the alternate path of the cycle.
4.  **Iteration:**
    * If the graph has cycles, remove an edge from a cycle.
    * Repeat this process until no cycles remain.
5.  **Conclusion:** The resulting graph is still connected (since we only removed redundant edges) and has no cycles. By definition, a connected graph with no cycles is a **tree**. This tree connects all vertices of $G$, making it a **spanning tree**.
